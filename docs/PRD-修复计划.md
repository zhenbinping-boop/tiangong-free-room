...
| 文档版本 | v1.3 |
| 创建日期 | 2026-09-11 |
| 当前状态 | **已完工（2026-09-13）；自 2026-09-13 起进入静默测试期** |
| 关联仓库 | `tiangong-free-room` |
| 关联文档 | `docs/代码审查报告`（同目录）、`docs/ANDROID-APP.md`（分发）、`README.md`（运行手册） |
...
| v1.2 | 2026-09-12 | D1（密码加密公式）、D3（数据接口与切换楼栋）、D4（验证码识别）经抓包与源码破译解决，待确认事项全部关闭；实测抓到两栋楼真实数据（92 + 86 = 178 间） |
| **v1.3** | 2026-09-13 | **P0–P4 全部实施完成**，逐阶段验收结论见 §10。新增：前端发布独立 workflow、iOS/微信安装引导、Android 分发（TWA → WebView 直壳）、站内自托管 APK。同步记录一项未达标项（触控目标尺寸）与静默测试期安排 |
...
- FR-1.2 **生产环境禁止生成 mock 数据**。伪造数据生成器仅在显式传入 `--allow-mock` 时可用
  （**实施结果：生成器与开关已整体删除 —— 比本要求更严格，见 §9.2**）
...
### P2 ✅ 已完成（2026-09-12） — 登录健壮性（v1.1 简化版）
...
### P4 ✅ 已完成（2026-09-13） — 上线与观察（v1.1 简化版）
...
- [x] 网站 `today.json` 的教室列表来自真实教务系统（数量与名称不再等于硬编码模板的 42 间）
      —— 实测 178 间（第一公共教学楼 92 + 第二公共教学楼 86），与硬编码模板 42 间无任何交集
- [ ] 连续 7 天 `source=live`，`updated_at` 为北京时间
      —— **进行中**：北京时间已正确，但截至 2026-09-13 仅有 1 天云端 `source=live` 记录。
      该项即静默测试期的唯一考核目标（见 §10.3）
- [x] 无需人工粘贴任何 cookie；单次运行内自动完成登录与抓取
      —— `TIANGONG_COOKIE` 与手写 Cookie 头已彻底移除，改为每次运行现场登录一次
- [x] 验证码识别失败时能自动重试（默认 10 次）并输出明确日志；达上限后非零退出
      —— 云端实测通过（2026-09-13 定时任务成功抓取）；服务端对错误验证码返回 `badCaptcha`
- [x] 抓取失败时：workflow 变红 + 数据保持上一次真实结果（`stale`）
      —— 「workflow 变红」已由 2026-09-13 首跑实际发生验证（失败于 git 同步步骤，非抓取）；
      「保留 stale 数据 + 前端非实时横幅」仅本地负向测试通过，**云端尚未实测**
- [x] 代码库内不存在任何会在生产环境生成伪造数据的路径
      —— 伪造数据生成器与 `--allow-mock` 开关**已整体删除**（全仓库 grep 无 mock 路径）；
      `crawler/verify_data.py` 在提交前强制校验 `source=live` 与数据日期
...
```bash
# 本地安装依赖（ddddocr 已在 requirements.txt 中）
pip install -r crawler/requirements.txt

# 运行抓取（失败即非零退出；本机需能访问教务系统）
...
# 位掩码单元测试
python crawler/test_bitmask.py

# 线上体检（比对线上与本地 web/index.html）
python tools/check_live.py
```

> **注意**：`--allow-mock` 参数**在当前代码中已不存在** —— 伪造数据生成器被整体删除，
> 而不是加开关保留（比 FR-1.2 的要求更严格）。本地没有凭据时无法产出数据，这是有意为之：
> 宁可不产出，也不产出无法追溯来源的数据。
...
| ~~`TIANGONG_COOKIE`~~ | ~~手工 cookie~~ | **已移除**（P2-4 完成），手工 cookie 模式彻底废弃 |
| `TZ` | 时区 | 设为 `Asia/Shanghai`（workflow 中已显式设置） |

---

## 10. 完工验收与静默测试期（v1.3 新增）

### 10.1 逐阶段验收结论

| 阶段 | 状态 | 证据 |
|---|---|---|
| P0 止血 | ✅ | `source` 三态字段、mock 改为显式 `--allow-mock`、失败非零退出、前端非实时横幅；仓库内 42 间假数据已清除，现为真实 178 间 |
| P1 正确登录 | ✅ | 新建 `crawler/jwxs_client.py`（原生 `j_spring_security_check` + 密码复合 md5 + ddddocr）；CAS 分支彻底删除；2026-09-12 真实跑通，2026-09-13 云端定时任务跑通 |
| P2 登录健壮性 | ✅ | 一次登录抓两栋楼；验证码识别失败换图重试（默认 10 次）；改用 cookie jar；`TIANGONG_COOKIE` 已移除 |
| P3 数据与调度 | ✅ | 数据接口与切换楼栋按真实 schema 实现（`position="{校区号}_{楼号}"`）；全链路 `Asia/Shanghai`；cron `0 22 * * *` = 北京 06:00；WebVPN 分支与 8 端点盲试移除 |
| P4 上线与观察 | ✅ | 调度时段已定；失败通知采用 GitHub 内置的 workflow 失败邮件（未额外接入钉钉 / Server 酱）；README 已补；7 天观察转入静默测试期 |

原 P3-1 计划产出的 `docs/api-schema.md` 已完成，接口真源以该文件为准；本文档 §8 D3 保留的是逆向推导过程记录。

### 10.2 本轮交付物（超出 PRD 原始范围，另见 §10.1）

- **前端发布解耦**：新增 `deploy-pages.yml`，push 到 main 且 `web/**` 有改动即发布，不再依赖 06:00 抓取任务顺带发布
- **PWA 安装引导**：覆盖 Android / iOS Safari / 微信内三种场景，横幅可「暂不」永久关闭，已安装形态不再打扰
- **Android 分发**：TWA 方案实测卡启动屏后弃用，改为 `android/webview-shell/`（系统 WebView 直壳）+ `build-apk.yml` 云端构建签名出包；APK 自托管于 `web/download/tiangong-rooms.apk`
- **文档**：`docs/ANDROID-APP.md`（分发全流程）、`README.md`（运行手册）、`docs/api-schema.md`（接口真源）

### 10.3 静默测试期（冻结自 2026-09-13；考核窗口 2026-09-14 → 2026-09-20）
...
> **为什么 2026-09-13 不计入 7 天窗口**：当天既有 06:00 的定时运行，也有 19:15 的手动触发运行，
> 不满足"无人工干预"。故 09-13 只作为**冻结起始日**，考核窗口为 **09-14 至 09-20**（7 天）。

**监督机制**（均为已上线的自动任务，无需人工值守）：
...
- 达标 → 2026-09-20 复查 09-14 至 09-20 共 7 天记录全为 `live`，关闭静默测试期，PRD 归档；
- 不达标 → 按失败日日志定位（登录 / 验证码 / 门禁 / git 同步四段），修复后重新计时。

### 10.4 遗留项与与要求的差异

**触控目标尺寸未达 `PRODUCT.md` 的 44×44px 要求**（2026-09-13 实测 `web/style.css`）：

| 状态 | 元素 |
|---|---|
| ✅ 达标 | 节次芯片 48px、楼栋分段控件 48px、搜索框 48px、下拉选择 44px、链接按钮 44px、主按钮 44px |
| ❌ 未达标 | 容量筛选 chip 36px、主题切换按钮 40px、安装按钮 40px、显示开关 40px |

- 影响面：与数据正确性、可用性无关，属无障碍体验项；
- 处置：静默测试期内**不动 UI**；期后二选一 —— ①调 CSS 补齐到 44px，或 ②下调该产品要求并在此记录理由。

**次要差异（非缺陷，如实记录）**：

| 项 | 要求 | 实现 | 判定 |
|---|---|---|---|
| 请求间隔（FR-8.1） | 请求间加**随机**间隔 | 验证码重试之间固定 `time.sleep(1)`，其余请求无间隔 | 非随机；但每日仅 1 次运行、单线程单账号，实际频率极低，风险可接受 |
| 验证码识别率 | 未量化 | 未做系统性统计。真机实测样本：2026-09-12 首次尝试即识别通过 | 单日样本不足以给出识别率结论，**不虚构数字** |
| 失败通知（P4-2） | Actions 邮件 / 钉钉 / Server 酱 任选其一 | 采用 GitHub 内置的 workflow 失败邮件（未额外接入） | 满足"至少一种"，但依赖 GitHub 账号的邮件设置，未实测收到过 |
